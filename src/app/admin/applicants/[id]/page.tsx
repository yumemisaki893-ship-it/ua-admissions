    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 font-display text-2xl font-semibold text-navy-900">{name}</h1>
          ...
          <Card>
            ...
            <CardContent className="grid gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
              <p className="flex items-center gap-2">
                ...
                <span className="font-medium text-navy-900">{formatDate(application.studentProfile.birthDate)}</span>
              </p>
              <p>
                <span className="text-muted-foreground">Gender:</span>{" "}
                <span className="font-medium text-navy-900">{application.studentProfile.gender}</span>
              </p>
              ...
              <p className="flex items-center gap-2">
                ...
                <span className="font-medium text-navy-900">{application.studentProfile.contactNumber}</span>
              </p>
              <p className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground">Email:</span>{" "}
                <span className="font-medium text-navy-900">{application.user.email}</span>
              </p>
          ...
          <Card>
            ...
            <CardContent className="text-sm">
              <p className="font-display text-base font-semibold text-navy-900">
                ...
                <span className="font-mono text-xs font-normal text-sky-600">({application.course.code})</span>
              </p>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                <Wallet className="h-5 w-5 text-sky-600" /> Payment
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm">
              {application.payment ? (
                <div className="grid gap-2 sm:grid-cols-3">
                  <p><span className="text-muted-foreground">Amount:</span>{" "}<strong className="text-navy-900">PHP {application.payment.amount.toLocaleString()}</strong></p>
                  ...
                  <div key={doc.id} className="flex items-center justify-between gap-3 rounded-lg border bg-card px-4 py-3">
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-navy-900">{meta.label}</p>
